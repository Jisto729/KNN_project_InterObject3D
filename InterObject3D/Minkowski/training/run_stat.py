...
def run_stat():
    ...
    for click in target_clicks:
        ...
        ap25 = (
            average_precision_score(y_true_25, conf_scores) * 100
            if y_true_25.sum() > 0
            else 0.0
        )
        ...
        ap50 = (
            average_precision_score(y_true_50, conf_scores) * 100
            if y_true_50.sum() > 0
            else 0.0
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--generate_scenes_kitti", action=argparse.BooleanOptionalAction, default=False
    )
    parser.add_argument(
        "--run_interaction", action=argparse.BooleanOptionalAction, default=False
    )
    parser.add_argument(
        "--run_stats", action=argparse.BooleanOptionalAction, default=False
    )

    ###############
    parser.add_argument(
        "--pretraining_weights", type=str, default="weights/weights_exp14_3_pointN.pth"
    )
    parser.add_argument(
        "--used_model", type=str, default="SmallHierarchicPointNetSeg"
    )  # MinkUNet34C, MinkUNet18B, MinkowskiPointNetSeg, HierarchicPointNetSeg, SmallHierarchicPointNetSeg
    parser.add_argument("--dataset", type=str, default="kitti")  # kitti
    ##############
